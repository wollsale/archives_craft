...
gulp.task("css", [ "scss"], () => {
  ...
  return gulp.src(pkg.globs.distCss)
    ...
    .pipe($.print())
    .pipe($.replace("url('../font/fontello.", "url('/font/fontello."))
    .pipe($.sourcemaps.init({loadMaps: true}))
...
// critical css task
gulp.task("criticalcss", ["css", "generate"], (callback) => {
...
// Default task
gulp.task("serve", ["set-dev-node-env", "css", "js"], () => {
...
gulp.task("generate", ["favicons", "fonts", "imagemin"], () => {
  $.fancyLog("-> Generating app assets");
});

// Test task
gulp.task("test", ["a11y"], () => {
  $.fancyLog("-> Runing tests...");
});

// Production build
gulp.task("build", ["set-prod-node-env", "static-assets-version", "download", "criticalcss", "js"]);
